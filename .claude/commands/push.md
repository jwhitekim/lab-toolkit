...
1. 위 변경사항을 분석하여 버전 범프 수준 판단
   ...
   - 하위 호환 불가 변경 → major
2. "버전을 X.X.X → X.X.X (minor/patch/major)로 올립니다. 진행할까요?" 확인 후 대기
3. 승인 시: `python bump.py <major|minor|patch>`
4. Conventional Commit 형식으로 메시지 생성 (한국어)
5. `git add -A`
6. `git commit -m "<생성된 메시지>"`
